...
MIT_Params = {
    ...
    "AK45-36": {
        ...
        # ⚠️ 未突合・実機未検証: my_ak45/control_mit_can/docs/ のNotionエクスポート仕様書は
        # ±45.0 rad/s と記載しており本値と食い違う。上記ERPMからの算出根拠も再計算すると
        # 本値(30.0)を再現できない。どちらが正しいか未確定。
        # 追記(2026-08-05): my_ak45/control_mit_can/docs_mit_can/ak45-36-firmware-and-parameters/
        # の実機エクスポート(45-36.McParams.McParams)で l_max_erpm=50000, motor_poles=14,
        # p_pid_ang_div=36 は実値と確認できたが、これらから30.0を導く計算式は依然再現できず、
        # 本値の直接的な根拠は見つかっていない。詳細は
        # .ai/logs/2026-08-05_03_ak45-36-firmware-export-crosscheck_01.md 参照。
        # 追記(2026-08-05): docs_mit_can/公式基本仕様.png（CubeMars公式基本仕様表）によると
        # 無負荷回転速度は52rpm@24V（出力軸側）＝約5.45 rad/s、定格回転速度は40rpm＝約4.19 rad/s。
        # 速度定数80rpm/V×24V÷減速比36≈53.3rpmで52rpmとほぼ整合しており、この52rpmの信頼度は高い。
        # 本値(30.0)・Notion仕様書(45.0)はいずれもこの無負荷速度の5〜8倍に達し、24V定格では
        # 物理的に到達し得ない。MITプロトコルのフィールドが表現できるエンコード範囲であって、
        # 定格・安全な連続動作速度とは別物と考えられる。実運用では公式仕様の52rpm(約5.45rad/s)を
        # 上限の目安にすべき。詳細は .ai/logs/2026-08-05_04_official-datasheet-crosscheck_01.md 参照。
        "V_max": 30.0,
