...
MIT_Params = {
    ...
    "AK45-36": {
        ...
        "V_min": -6.0,
        # 元々2026-02-19時点(コミット54b7f22)では 'V_max': 6.0 # モーター速度（rad/s）マニュアルより
        # という、マニュアル参照の値だった。翌日2026-02-20(コミット172017f「パラメータ設定 決定」)
        # で「50000 ERPM / (14極対 * 36減速比) から算出される実用域」として自己計算した30.0に
        # 上書きされたが、2026-08-05の調査でこのERPM計算式自体を再現できないことが判明していた
        # （旧コメント参照、my_ak45/control_mit_can/docs/ のNotionエクスポート仕様書±45.0 rad/sとも
        # 食い違う。詳細は .ai/logs/2026-08-05_03_ak45-36-firmware-export-crosscheck_01.md）。
        # docs_mit_can/公式基本仕様.png（CubeMars公式基本仕様表）の無負荷回転速度52rpm@24V
        # （出力軸側）＝約5.45 rad/s（詳細は .ai/logs/2026-08-05_04_official-datasheet-crosscheck_01.md）
        # に近い値として、旧30.0のままでは妥当性を欠くと分かっていた。
        # 追記(2026-08-11): 実機3台(exp_003_multi_motor.py)でのインピーダンス制御試験中、SafetyMonitorが
        # 繰り返し速度超過(6 rad/s超)を誤検知することを受けて調査。ログの位置(output_angle)の
        # 有限差分と、CAN経由でデコードされた速度(output_velocity)を比較したところ、3台・複数時刻に
        # わたって速度値が有限差分の約5.2〜7.5倍（平均して約5.5〜6倍）過大に出ていることを確認した。
        # 30.0 / (5.5〜6倍) ≈ 5.0〜5.5 rad/s となり、上記の公式無負荷速度5.45 rad/s、および最初の
        # マニュアル値6.0の両方とよく一致する。詳細は
        # .ai/logs/2026-08-11_05_v_max_correction_01.md 参照。
        # これを受け、V_min/V_maxを元のマニュアル値6.0（符号対称）に戻す。
        "V_max": 6.0,
        "T_min": -32.0,  # 最大電流 35A 時の理論最大トルク付近
        # ⚠️ 未突合・実機未検証: 同じ最大トルク/電流相当値として、Notionエクスポート仕様書は
        # ±18.0 Nm、demos/mit_can/demo_full_state_feedback_mit_can.py は 3.7 Nm と記載しており、
        # 本値を含め3箇所で一致しない。どれも実測で検証されていない。
        # 追記(2026-08-05): 実機エクスポートの l_current_max=35A と突合でき、本値(32A)はそこから
        # 妥当な安全マージンを取った値として裏付けが得られた（この値自体は電流[A]であり、
        # Notion仕様書のNmとは単位が異なる点に注意）。Notion仕様書の18.0はAK80-9自身の
        # T_max(=18.0、下記 "AK80-9" エントリ参照、これもA)と数値が一致しており、モータ種別を
        # 取り違えて転記された可能性が高い。demoの3.7は T_max(32A)×Kt_TMotor(0.1206)≈3.86Nm
        # （ギア前・モーター側トルク）に近く、この式のやや古い値と推測されるが未検証。詳細は
        # .ai/logs/2026-08-05_03_ak45-36-firmware-export-crosscheck_01.md 参照。
        # 追記(2026-08-05): docs_mit_can/公式基本仕様.png によると定格電流2A・ピーク電流6.5A
        # （表記「ADC」の単位解釈には留保が必要だが、定格トルク8Nm・ピークトルク24Nmとの整合が
        # 取れているためモーター側電流[A]とみて矛盾はない）。本値(32A)はこの公式ピーク電流の
        # 約5倍であり、firmwareのl_current_max=35Aとの整合はあるが、公式の定格・ピーク電流とは
        # 大きく乖離している。本フィールドはMITプロトコルのコマンド範囲であって、実際に常用してよい
        # 電流ではない可能性が高い。実運用では公式仕様のピーク電流6.5A付近を上限の目安にすべき。
        # 詳細は .ai/logs/2026-08-05_04_official-datasheet-crosscheck_01.md 参照。
        "T_max": 32.0,
        "Kp_min": 0.0,
        "Kp_max": 500.0,
        "Kd_min": 0.0,
        "Kd_max": 5.0,
        # ⚠️(2026-08-05) 実機エクスポート(45-36.McParams.McParams)の foc_current_kp と完全一致することを確認。
        # つまりこの値はトルク定数[Nm/A]ではなく、FOC電流制御ループの比例ゲイン（物理的に無関係な値）を
        # 数値が似ているという理由で流用したものと判明した。「暫定採用」ではなく「別物の値の誤用」に近い。
        # firmwareの foc_motor_flux_linkage(0.005314 Wb) と極対数から Kt=1.5*極対数*flux_linkage で
        # 概算すると、14極対なら約0.112、7極対なら約0.056 Nm/Aとなる（式の係数・規約は未検証の参考値）。
        # 詳細は .ai/logs/2026-08-05_03_ak45-36-firmware-export-crosscheck_01.md 参照。
        # 追記(2026-08-05): docs_mit_can/公式基本仕様.png に公式トルク定数「0.11 Nm/A」の記載を発見。
        # 本値(0.1206)との差は約9.6%。また同表の極対数は14と明記されており、上記flux linkageからの
        # 概算で14極対を仮定した場合(約0.112)にも近い。定格電流2A×0.11×減速比36≈7.9Nm≈定格トルク8Nm
        # と高い整合性があり、0.11 Nm/Aはモーター側（ギア前）トルク定数として信頼度が高いと考えられる。
        # 本値は流用元が誤りだったにもかかわらず、結果的に公式値の1割以内に収まっていたことになる。
        # 詳細は .ai/logs/2026-08-05_04_official-datasheet-crosscheck_01.md 参照。
        "Kt_TMotor": 0.1206,  # foc_current_kp の値と一致
        "Current_Factor": 0.59,
        "Kt_actual": 0.1206,  # 実測パラメータがないためKp値を暫定採用
        "GEAR_RATIO": 36.0,  # p_pid_ang_div より確定（2026-08-05: 実機エクスポートのp_pid_ang_div=36と完全一致を再確認）
...
class CAN_Manager(object):
    ...
    def parse_MIT_message(self, data, motor_type):
        ...
        if len(data) == 8:
            # temp はオフセット・スケールなしの生バイト（0〜255）をそのまま℃として扱う。
            # 2026-08-11の実機検証（AK45-36 3台、アイドル状態）でこの値（65〜75℃）と外装の
            # 体感温度（ほんのり温かい程度）に乖離が見つかったが、リポジトリ内のドキュメント・
            # ファームウェアエクスポートのどこにもオフセットの根拠は見つからなかった
            # （servo_can.py も同じ生バイト方式）。詳細は
            # .ai/logs/2026-08-11_02_temperature-decode-verification_01.md 参照。断定できる
            # 根拠がないため、この行自体は変更していない。
            temp = int(data[6])
            error = int(data[7])
