...
14. [x] `identification/csv_adapter.py` を作成。`wall_time` を時刻軸に使い、起動過渡0.15sを
        ...
        **実測列のずらし量は2行**（`identification/identify.py` の `DEFAULT_SHIFT`）:
        `sysid_run_check.py` が相互相関から測る3サンプル分の遅れのうち1サンプルは
        MuJoCo の rollout が `sensor[i] = ctrl[0..i-1] への応答` という因果性として
        構造的に持っているため、追加で補正すべきは 3−1=2 サンプルになる。
        実際に振ると最終コストは shift=0:0.665 / 1:0.408 / **2:0.334** / 3:0.344 で2が最小
