...
14. [x] `identification/csv_adapter.py` を作成。`wall_time` を時刻軸に使い、起動過渡0.15sを
        ...
        **実測列のずらし量は2行**（`identification/identify.py` の `DEFAULT_SHIFT`）。
        ...
        CSVを静的に扱う限り、正しい補正量は上2つの合計＝**約3行**である。それでも
        `DEFAULT_SHIFT` が2なのは、残る約1行を MuJoCo 側が暗黙に補正しているため:
        `sysid.sysid_rollout` が返す `sensordata` は `data[0]` が「初期状態そのもの
        （1ステップも進めていない値）」でありながら `times[0] = dt` と付けられており
        （実測で確認済み）、`sysid.model_residual` はこの `times` を使って実測を窓がけ・
        補間するため、予測は自分の物理時刻より約1サンプル後の実測と突き合わされる。
        よって明示的に詰めるのは 3 − 1 = **約2行**。実際に振ると最終コストは
        shift=0:0.665 / 1:0.408 / **2:0.334** / 3:0.344 で2が最小となり、この勘定と一致した。

        > **注**: この暗黙の1サンプルは `mujoco.sysid` の実装詳細に依存する。mujoco の
        > バージョンを上げて `sensordata` の時刻付けが変わった場合は、上記のスイープを
        > やり直すこと。
