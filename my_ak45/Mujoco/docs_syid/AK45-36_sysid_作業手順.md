...
17. [~] **一部完了（PC単独部分のみ）**。`identification/validate.py` で
        ...
        **未完了の部分**: ここで使った検証ランは同定ランと同じ multi-sine 励振である。
        「sysidに使った軌道とは**別の**軌道」（PD制御での位置追従等）での検証はまだできていない。
        `my_ak45/control_mit_can/logs/` は `.gitignore` 対象でPC側に1本も存在しないため、
        これはPiでの追加データ取得が必要（→ 項目19・フェーズ1）
...
19. [ ] 別軌道（PD制御・インピーダンス制御）のデータをPiで取得する。
