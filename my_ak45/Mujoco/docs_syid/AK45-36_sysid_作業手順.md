...
17. [~] **一部完了（PC単独部分のみ）**。`identification/validate.py` で
        **leave-one-run-out 交差検証**を実施した（採用3ランを「2本で同定 → 残り1本で評価」と
        3通り回す）。学習コストは対象パラメータを増やせば必ず下がるので、ステージ2と3の
        どちらを採るかは同定に使っていないデータでの誤差でしか決められない。
        評価指標は物理単位のRMS誤差（3分割の平均）:

        | ステージ | 学習コスト | 位置RMS [mrad] | 速度RMS [rad/s] | 位置誤差の最大 [mrad] |
        |---|---|---|---|---|
        | 同定前（XMLの初期値） | — | 265.10 | 1.1549 | 474.8 |
        | 1: armature | 10.511 | 143.07 | 0.7135 | 297.7 |
        | 2: +frictionloss | 0.223 | 16.51 | 0.1063 | 77.8 |
        | **3: +damping** | **0.143** | **11.58** | **0.0879** | **37.5** |

        → **ステージ3（damping込み）を採用する**。未知データでも位置RMSが30%・最大誤差が52%
        改善しており、`damping` は学習データのノイズを拾っているのではなく実機の挙動を
        説明している。参照手法(RS02)の「有意な粘性摩擦は見えない」という記述は
        このモーターには当てはまらなかった。

        **未完了だった部分（→ 項目19・20a で解消済み）**: ここで使った検証ランは同定ランと
        同じ multi-sine 励振である。「sysidに使った軌道とは**別の**軌道」での検証は
        項目19（Piでのデータ取得）と項目20a（`validate_trajectory.py`）で実施した。
        結論（ステージ3採用）は別軌道でも覆らなかったが、低速域ではステージ2の方が
        良いという条件依存性が見つかっている（→ 項目20a）
18. [x] 境界張り付きなし・分割間のばらつきも小さいことを確認した。
        3分割それぞれで同定された値のばらつき:

        | ステージ | パラメータ | 平均 ± 標準偏差 | 幅 |
        |---|---|---|---|
        | 2 | armature | 0.012497 ± 0.000007 | 0.1% |
        | 2 | frictionloss | 0.159800 ± 0.000871 | 1.3% |
        | 3 | armature | 0.012751 ± 0.000015 | 0.3% |
        | 3 | frictionloss | 0.097470 ± 0.002873 | 6.5% |
        | 3 | damping | 0.027136 ± 0.001014 | 9.1% |

        `armature` はどのステージ・どの分割でも極めて安定。`frictionloss` と `damping` は
        ステージ3で互いに一部代替可能なため単独では±10%程度動くが、**両者の組は汎化性能を
        改善している**ので「複数解の曖昧さ」は実用上問題にならない
19. [x] 別軌道（PD制御・インピーダンス制御）のデータをPiで取得する。
        項目17の未完了部分を埋めるために必要。条件を変えた追加データ
        （負荷変更・正逆両方向・振幅違い）もここで検討する → フェーズ1に戻る
        
        **実施内容（2026-08-20）**: exp_008（単発版）・exp_009（複数試行版）を実装。
        exp_009で5試行のインピーダンス制御データを取得完了（全試行緊急停止なし）。
        ランダムサンプリングされた多様なパラメータ条件（K: 8.3-18.4, B: 0.2-0.9, 
        振幅: 0.44-0.97 rad）で検証用データが充足された（→ 次は項目20a参照）

        **追記（2026-08-20）**: 取得データの自動品質チェック `validation_run_check.py` を
        追加し、exp_008/exp_009 の実行末尾から自動で呼ぶようにした（exp_005 →
        `sysid_run_check.py` と同じ方針）。指令トルクが存在しない閉ループデータのため
        `sysid_run_check.py` の11項目のうち4項目（トルク線形性・符号反転・指令-実測の遅れ・
        周波数応答）は移植できず、代わりに検証データ特有の項目（追従品質・飛びつき過渡・
        PC側が切り出せる検証区間数・双方向カバレッジ）を持つ。取得済み5試行の判定は
        **条件付き合格（WARN 5件）**で、うち2件は **飛びつき時のピーク速度が
        `safety.max_velocity` の83〜85%** に達していたというもの（→ 下記「未対応」）

### フェーズ4.5: 別軌道での validation 実装 【Windows PC】

20a. [x] `identification/validate_trajectory.py` を新規作成し、別軌道（exp_009）での
        モデル検証を実施した。exp_005 の3ラン全部で同定 → exp_009 の5試行で評価する
        （`validate.py` の leave-one-run-out とは違い、同定側は3ラン固定）。
        5試行の平均:

        | ステージ | 位置RMS [mrad] | 速度RMS [rad/s] | 位置最大 [mrad] | 位置RMS/振れ幅 |
        |---|---|---|---|---|
        | 同定前（XMLの初期値） | 593.8 | 3.2990 | 1551.0 | 110.4% |
        | 1: armature | 343.7 | 1.9583 | 920.4 | 65.1% |
        | 2: +frictionloss | 83.2 | 0.4189 | 222.6 | 12.9% |
        | **3: +damping** | **63.3** | 0.3341 | 217.5 | 13.8% |

        → **項目17の結論（ステージ3を採用）は別軌道でも覆らなかった**。平均で見れば
        ステージ3が最良（同定前比9.4倍改善、ステージ2比1.3倍）。同定側の入力トルクを
        `output_torque` に揃えた場合（`--fit-torque output_torque`）でも順位は同じで、
        ステージ2→3の改善倍率も 6.7倍→8.2倍 と同傾向だった。

        **ただし試行ごとに見るとステージ2と3の優劣は割れる**（位置RMS [mrad]）:

        | 試行 | 振幅 [rad] | 周期 [s] | K | ステージ2 | ステージ3 |
        |---|---|---|---|---|---|
        | 0 | 0.877 | 2.15 | 9.13 | 150.2 | **30.9** |
        | 1 | 0.435 | 6.06 | 18.38 | **54.2** | 144.7 |
        | 2 | 0.966 | 2.18 | 8.28 | 138.5 | **19.4** |
        | 3 | 0.540 | 3.19 | 14.75 | 52.2 | **33.7** |
        | 4 | 0.467 | 5.54 | 17.14 | **20.8** | 88.0 |

        速い試行（周期2秒台）ではステージ3が圧勝、遅い試行（周期5〜6秒台）では
        ステージ2の方が良い。ステージ3は摩擦の一部をクーロン（`frictionloss`
        0.160→0.098）から粘性（`damping` 0→0.027）へ移す解なので、粘性項がほとんど
        効かない低速域ではクーロン摩擦を過小評価することになる。**低速域を主に扱う
        用途では、平均値だけを見てステージ3を選ぶのは早い**。

        **絶対値は項目17の数字（位置RMS 11.6 mrad）と直接比較できない**: 検証データが
        100Hz記録・閉ループ・入力トルクが `output_torque` 固定と条件が違う。ここで
        読むべきは同定前からの改善倍率とステージ間の順位。位置RMSが軌道の振れ幅の
        13%程度残っている点は、モデルの限界（トルク-速度特性・非線形摩擦を持たない）
        と `Kt_actual` の既知の誤差（公式比+10%）の両方を含んだ値である

### フェーズ5: 反映

20b. [x] 同定結果をMuJoCoモデル（XML）に反映し、`my_ak45/Mujoco/` 配下に保存・コミットする。
...
- ~~`my_ak45/Mujoco/data/raw/` のCSVはサンプルレート次第で1ファイルあたり数百KB〜数MB程度に
  なりうる（1kHz×10秒＝10,000行）。試行数が増えた場合にリポジトリサイズへの影響を再検討する~~
  → **2026-08-13に整理**: 採用トリオとFAIL判定の実例1本だけを残し、他7ラン（約8MB）を削除。
  作業ツリーは14MB→5.6MBになった。ただし削除したファイルのblobはgit履歴に残るため
  `.git` のサイズは減らない。今後は「採用しないランはコミットせずPi側で捨てる」運用にする
  （フェーズ1 項目5）。保持ランの一覧は `data/raw/README.md` 参照
- ~~Windows PC側の `mujoco[sysid]` インストール方法~~ →
  ...
  **2026-08-13に再確定**: `pyproject.toml` `[dependency-groups]` の `mujoco-sysid` として
  ...
  → **バージョン固定も完了（同日）**: 下限が `>=3.5.0` のままだと、このプロジェクトの
  `requires-python = ">=3.9"` のせいで Python 3.9 環境では uv が mujoco **3.5.0** 側に
  解決してしまう（3.11.0 の Requires-Python が `>=3.10` のため）。3.5.0 はsdistのみで
  ホイールが無く、動作確認も 3.11.0 でしか取れていない。
  `"mujoco[sysid]>=3.11.0; python_version >= '3.10'"` に変更し、`uv.lock` の mujoco が
  3.5.0/3.11.0 の2エントリから 3.11.0 単一に統一されたことを確認済み。
  マーカーで逃がしているのは、実機(Pi)側で動くメインパッケージの `requires-python >= 3.9` を
...
- ~~**`frictionloss` と `damping` は部分的に識別不能**（ステージ3で `damping` を足すと
  ...
  leave-one-run-out 交差検証で、同定に使っていないランに対する位置RMSがステージ2の
  16.51 mrad からステージ3で 11.58 mrad に改善したため、**ステージ3（damping込み）を採用**。
  2つのパラメータが互いに一部代替可能なこと自体は変わらない（分割間で `frictionloss` ±6.5%、
  `damping` ±9.1% 動く）が、組として汎化性能を上げているので実用上の問題はない。
  `armature` はこの影響をほぼ受けない（±0.3%）
...
- ~~フェーズ3の軌道分割（0.5〜1秒区間、`ModelSequences` への複数シーケンス投入）は方針決定のみで
  実装は未着手~~ → **2026-08-13に実装済み**（`identification/csv_adapter.py`）。
  ただし区間長の最適値は未確定のまま（0.5s と 1.0s しか試しておらず、
  4.5%〜8.8%の再現不能ずれのどこで手を打つかは検証していない）
