...
# 実測列を何行前に詰めるか。
#
# sysid_run_check.py は指令と実測の相互相関から「実測列を3行前に詰めること」と報告する。
# 一方 exp_005_sysid_excitation.py のコメントは「1行前に詰めること」としている。両者は
# 矛盾しているように見えるが、MuJoCo の rollout が sensor[i] = ctrl[0..i-1] への応答 という
# 因果性を構造的に持つ（＝1サンプル分の遅れは補正済み）ことを踏まえると、
# rollout ベースのフィットで追加すべき補正は 3 - 1 = 2 サンプルになる。
#
# 実際に stage2・desired_torque・区間0.5s で shift を振ると最終コストは
#   shift=0: 0.665 / shift=1: 0.408 / shift=2: 0.334 / shift=3: 0.344
# となり、上記の理屈どおり2で最小になることを確認した。
