...
# 別々の量**である。次の2段階で整理できる。
#
# ■ 第1段階: 実機CSV上の、指令と実測のずれ = 合計 約3サンプル
#
...
#
#   合計 (a)+(b) ≈ 2.9 サンプル。sysid_run_check.py が相互相関で測る「3行」はこの合計で
#   あり、実機CSVを静的に扱う限りこれが正しい補正量である。
#
# ■ 第2段階: そのうち約1サンプルは MuJoCo 側が既に供給している → 明示的に詰めるのは2行
#
#   sysid.sysid_rollout が返す sensordata は、data[0] が「初期状態そのもの（1ステップも
#   進めていない値）」でありながら times[0] = dt と付けられている（実測で確認済み）。
#   sysid.model_residual はこの times を使って実測を窓がけし予測を補間するため、
#   予測は自分の物理時刻より約1サンプル後の実測と突き合わされる。つまり約1サンプル分の
#   前詰めが暗黙に入っている。
#   → 明示的に詰めるべきは 3 − 1 = 約2サンプル。
#
# 実際に stage2・desired_torque・区間0.5s で shift を振ると最終コストは
#   shift=0: 0.665 / shift=1: 0.408 / shift=2: 0.334 / shift=3: 0.344
# となり2で最小で、上の勘定と一致する。
#
# 注: この暗黙の1サンプルは sysid 側の実装詳細に依存する。mujoco のバージョンを上げた
# 際に sensordata の時刻付けが変わると最適な shift も変わりうるので、その場合は
# 上記のスイープをやり直すこと。
#
