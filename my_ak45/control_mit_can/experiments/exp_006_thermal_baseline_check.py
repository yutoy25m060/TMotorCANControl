...
安全上の注意:
- config.yaml の motor.max_temp（50℃）をそのまま使うと、このスクリプトの目的である
  「65℃前後の実測」自体が update() の温度チェックで即座に RuntimeError になってしまう。
  そのため本スクリプトに限り、上限を実機ファームウェアの実測しきい値
...
LOG_FILE = make_log_path("exp006_thermal_baseline")
