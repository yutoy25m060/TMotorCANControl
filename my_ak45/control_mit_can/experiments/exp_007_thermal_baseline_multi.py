...
安全上の注意（exp_006と同じ方針）:
- 本スクリプト作成時点の config.yaml の motors[].max_temp は各50℃で、そのまま使うと
  観測したい65〜71℃前後の実測値自体が update() の温度チェックで即座に RuntimeError に
  なっていた（現在は exp_006/exp_007 の観測結果を受けて各75℃に引き上げ済み。
  config.yaml 冒頭のコメント参照）。本スクリプトは config.yaml の値に依存せず、上限を
  実機ファームウェアの実測しきい値（l_temp_motor_start=85℃）に合わせて85℃に
  引き上げている。config.yaml のmax_tempは変更していない。
...
# 実行フォルダ（logs/exp007_thermal_baseline_multi_{timestamp}/）を作成し、CSV・コンソールログをまとめる
RUN_DIR = make_run_dir("exp007_thermal_baseline_multi")
